...
describe('decodeTroubleCodes / decodeDtcResponse', () => {
  ...
  it('appends continuation frames that do not repeat the service byte', () => {
    ...
    expect(decodeDtcResponse(['43 03 01 04', '20 05 30'], 0x43)).toEqual([
      'P0301',
      'P0420',
      'P0530',
    ]);
  ...
  it("decodes each ECU's response separately when several reply", () => {
    ...
    expect(
      decodeDtcResponse(['43 01 03 01', '43 01 04 20'], 0x43, { skipCountByte: true })
    ).toEqual(['P0301', 'P0420']);
  });
  it('skips the CAN count byte so it does not become a phantom code', () => {
    ...
    expect(decodeDtcResponse(['43 02 03 01 04 20'], 0x43, { skipCountByte: true })).toEqual([
      'P0301',
      'P0420',
    ]);
  });
  it('strips an ISO-TP frame index before decoding', () => {
    expect(decodeDtcResponse(['0: 43 02 03 01 04 20'], 0x43, { skipCountByte: true })).toEqual([
      'P0301',
      'P0420',
    ]);
  });
