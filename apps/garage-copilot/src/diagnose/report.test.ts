...
describe('buildReport', () => {
  ...
  it('converts live values to imperial when requested', () => {
    const snap = {
      ...snapshot,
      livePids: [{ pid: '05', label: 'Engine Coolant Temperature', value: 100, unit: 'C' }],
    };
    expect(buildReport(snap).text).toContain('Engine Coolant Temperature: 100 C');
    expect(buildReport(snap, undefined, undefined, 'imperial').text).toContain(
      'Engine Coolant Temperature: 212 F'
    );
  });
